...
bool AllowMinDifficultyForBlock(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if (!params.fPowAllowMinDifficultyBlocks)
        return false;
    // Vitcoin: Magic number at which reset protocol switches
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // Vitcoin: Special rules for minimum difficulty blocks with Digishield
    ...
    return CalculateVitcoinNextWorkRequired(pindexLast, pindexFirst->GetBlockTime(), params);
}
...
{
    // POW VALIDATION DISABLED - Always return true for custom private network
    // Original validation logic has been bypassed to allow custom genesis block
    // and simplified block acceptance on private network
    return true;
