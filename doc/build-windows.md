...
Below are some notes on how to build Vitcoin Core for Windows.
...
To build Vitcoin from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the Vitcoin GitHub repository](https://github.com/vitcoin/vitcoin/)).
...
Note that for WSL v1 the Vitcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/vitcoin, AND not under, for example, /mnt/d/vitcoin.
...
```
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Vitcoin's dependencies):
```bash
...
way. This will install to `c:\workspace\vitcoin`, for example:
```bash
make install DESTDIR=/mnt/c/workspace/vitcoin
```
