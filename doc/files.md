...
The data directory is the default location where the Vitcoin Core files are stored.
...
Windows  | `%APPDATA%\Vitcoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `vitcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `vitcoin.conf`       | User-defined configuration settings for `vitcoind` or `vitcoin-qt`; can be specified by `-conf` option
`./`               | `vitcoind.pid`        | Stores the process ID (PID) of `vitcoind` or `vitcoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `vitcoind` or `vitcoin-qt`
...
`vitcoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Vitcoin Core:
