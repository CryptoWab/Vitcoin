Vitcoin Core version 1.14.7 is now available from:

  <https://github.com/vitcoin/vitcoin/releases/tag/v1.14.7/>

This is a new minor version release, including enchancements to several RPC
methods and important security updates for Vitcoin-Qt. Vitcoin-Qt users on
...
To receive notifications about updates, subscribe to the release mailing list:
  ...
  <https://sourceforge.net/projects/vitcoin/lists/vitcoin-releases>


Compatibility
==============

Vitcoin Core is extensively tested on Ubuntu Server LTS, macOS and Windows.
...
This release contains fixes for Vitcoin Qt across all platforms that increase
security for end users.

### Disable BIP-70 payment server by default

To mitigate future potential risk inside Vitcoin Qt, support for BIP-70 payment
...
inside Vitcoin Qt. By default, incoming BIP-70 payment requests and files or
...
vitcoin.conf or as an argument to Vitcoin-Qt if absolutely needed. Please use
extreme caution when exercising this option.

This is step 1 in full deprecation of BIP-70, BIP-71 and BIP-72 inside Vitcoin
...
* Updated the `rpcuser.py` script to improve the password and salt generation
  ...
  current authentication in `vitcoin.conf` with new credentials generated with
...
Fee estimation was explicitly not ported or supported on Vitcoin Core since
...
to support Vitcoin parametrization.
...
```
vitcoin-cli help getblockstats
```
...
* Updated the python `ltc-scrypt` module to a maintained fork (#3080), which
  can be found at https://github.com/vitcoin/ltc-scrypt
