Vitcoin Core version 1.14.6 is now available from:

  <https://github.com/vitcoin/vitcoin/releases/tag/v1.14.6/>

This is a new minor version release, including important security updates and
changes to network efficiency. All Vitcoin Core users - miners, services,
...
Vitcoin network from 1 VTC to 0.01 VTC. The full recommendation can be found
...
1. The wallet will attempt to not create any outputs smaller than 0.03 VTC as
   change if possible.
2. If after signing there is more than 0.01 VTC left in the change output, the
   wallet will keep the output as-is.
3. If the change output drops under 0.01 VTC, the wallet will discard the
   change and instead give it to miners as fee.
4. Any output under 0.01 VTC will not be accepted as valid by default.
...
```
vitcoin-cli addnode <address> remove
```
...
```
vitcoin-cli liststucktransactions
```
...
```
vitcoin-cli setmaxconnections <desired_amount>
```
...
```
vitcoin-cli rescan <start_block>
```
...
efforts to make Vitcoin more accessible to people that do not speak English.
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by Vitcoin Core.
