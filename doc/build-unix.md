...
You must install required dependencies to build a basic Vitcoin daemon, optional dependencies may vary according to your requirements.
...
Vitcoin Core's released binaries are built using the "depends system", which
...
**Optional dependencies** :
```bash
# Qt (required for vitcoin-qt GUI)
...
According to installed dependencies, the following steps will compile `vitcoind`, `vitcoin-cli` and `vitcoin-qt`.
...
#### Enable Qt GUI
Create `vitcoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Vitcoin Core expects BerkeleyDB 5.3**.
...
Otherwise, you can build Vitcoin Core from self-compiled [depends](/depends/README.md).

**Note**: You only need Berkeley DB if the wallet is enabled (see [*Disable-wallet mode*](#disable-wallet-mode)).

#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Vitcoin may be compiled in
...
#### Security
To help make your Vitcoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Vitcoin should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling Vitcoin Core.
