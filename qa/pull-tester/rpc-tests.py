...
# Copyright (c) 2022-2024 The Vitcoin Core developers
...
if 'ENABLE_WALLET' not in vars():
    ENABLE_WALLET=0
if 'ENABLE_VTCCOIND' not in vars():
    ENABLE_VTCCOIND=0
...
#Set env vars
if "VTCCOIND" not in os.environ:
    os.environ["VTCCOIND"] = BUILDDIR + '/src/vitcoind' + EXEEXT
...
if not (ENABLE_WALLET == 1 and ENABLE_UTILS == 1 and ENABLE_VTCCOIND == 1):
    print("No rpc tests to run. Wallet, utils, and vitcoind must all be enabled")
    sys.exit(0)
...
class RPCTestHandler:
    ...
    def __init__(self, num_tests_parallel, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie vitcoinds, we can apply a
...
class RPCCoverage(object):
    ...
    commands per `vitcoin-cli help` (`rpc_interface.txt`).
